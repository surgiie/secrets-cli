...
**Note** That options that have variables that are empty, e.g `--key "$empty_variable"` will be parsed as true/boolean flag. To avoid unintended data from being created, consider running the cli in strict mode:

```bash
bash -euo pipefail -c 'secrets new mysecret --mysecret "$empty_variable"'
```

This will cause the cli to exit with an error if the variable is empty.
